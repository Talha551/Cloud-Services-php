...
<?php else: ?>
    <div class="bg-[#13151f] border border-[#2a2d3e] rounded-xl overflow-hidden">
        <div class="grid grid-cols-7 px-5 py-3 text-xs uppercase tracking-wide text-slate-500 border-b border-[#2a2d3e]"><div>Invoice #</div><div>Amount</div><div>Due Date</div><div>Status</div><div>Provision</div><div>Payment</div><div>Action</div></div>
        <?php foreach ($rows as $row): ?>
            <?php $status = strtolower((string) $row['status']); $cls = ($status === 'paid') ? 'bg-green-500/15 text-green-400 border-green-500/30' : 'bg-blue-500/15 text-blue-400 border-blue-500/30'; ?>
            <div class="grid grid-cols-7 px-5 py-3 text-sm border-b border-[#1e2130] last:border-0"><div class="text-slate-400"><?php echo (int) $row['id']; ?></div><div class="text-slate-300">$<?php echo number_format((float) $row['total'], 2); ?></div><div class="text-slate-400"><?php echo html_escape(substr($row['created_at'], 0, 10)); ?></div><div><span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border <?php echo $cls; ?>"><?php echo html_escape($row['status']); ?></span></div><div class="text-slate-400"><?php echo html_escape(isset($row['provisioning_status']) && $row['provisioning_status'] ? $row['provisioning_status'] : 'awaiting_payment'); ?></div><div class="text-slate-400 text-xs"><?php echo !empty($row['transaction_id']) ? html_escape((string) $row['payment_method'].' | '.(string) $row['transaction_id']) : '-'; ?></div><div>
