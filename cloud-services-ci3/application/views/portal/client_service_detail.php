<div>
    <div class="flex items-center gap-3 mb-6">
        <a href="<?php echo site_url('client/services'); ?>" class="p-1.5 rounded-lg hover:bg-[#1e2130] text-slate-400 hover:text-slate-200 transition-colors">
        ...
        <div class="flex-1">
            ...
            <p class="text-sm text-slate-500 mt-1">Service ID: <?php echo (int) $service['id']; ?></p>
        </div>
    </div>
    ...
    <?php $providerBusy = !empty($provider_is_processing); ?>
    <?php if ($providerBusy): ?>
    ...
    <?php if ((int) (isset($service['provider_server_id']) ? $service['provider_server_id'] : 0) <= 0): ?>
        <div class="mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
            This service is local-only right now. Click <strong>Provision on Provider</strong> to create and link it with SolusVM.
        </div>
    <?php endif; ?>
    ...
    <?php
        ...
        $app_access_map = array(
            ...
            'da '           => array('DirectAdmin',       2222,  '/',              'http',  'text-blue-400'),
            ...
            'froxlor'       => array('Froxlor',           80,    '/froxlor',       'http',  'text-indigo-400'),
            'wordpress'     => array('WordPress Admin',   80,    '/wp-admin',      'http',  'text-blue-300'),
            'nextcloud'     => array('Nextcloud',         80,    '/',              'http',  'text-sky-400'),
            'owncloud'      => array('ownCloud',          80,    '/',              'http',  'text-sky-400'),
            'joomla'        => array('Joomla Admin',      80,    '/administrator', 'http',  'text-red-400'),
            'drupal'        => array('Drupal Admin',      80,    '/admin',         'http',  'text-blue-400'),
            'magento'       => array('Magento Admin',     80,    '/admin',         'http',  'text-orange-300'),
            'opencart'      => array('OpenCart Admin',    80,    '/admin',         'http',  'text-green-300'),
            'prestashop'    => array('PrestaShop Admin',  80,    '/admin',         'http',  'text-pink-400'),
            'openvpn'       => array('OpenVPN Access',    943,   '/admin',         'https', 'text-amber-400'),
            'woocommerce'   => array('WordPress Admin',   80,    '/wp-admin',      'http',  'text-purple-300'),
            'gitlab'        => array('GitLab',            80,    '/',              'http',  'text-orange-400'),
            'moodle'        => array('Moodle Admin',      80,    '/admin',         'http',  'text-green-400'),
            'panel'         => array('Control Panel',     8080,  '/',              'http',  'text-slate-300'),
        );

        $detected_access = array(); // array of detected apps with their access info
        $app_search_name = strtolower(trim((string) $resolvedAppName));
        // Use direct login link from SolusVM if available (most reliable)
        ...
        if ($app_search_name !== '' && $resolvedIp !== '') {
            foreach ($app_access_map as $fragment => $info) {
                if (strpos($app_search_name, $fragment) !== false) {
                    ...
                    $port  = $info[1];
                    $path  = $info[2];
                    // Prefer SolusVM's direct link; fall back to guessed URL
    ...
    <div class="mb-6 bg-[#13151f] border border-[#2a2d3e] rounded-xl p-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div class="flex items-center gap-3 p-3 bg-[#0f1117] rounded-lg border border-[#2a2d3e]">
                ...
                <div class="min-w-0">
                    ...
                    <button type="button" onclick="copyText('<?php echo html_escape($resolvedIp); ?>:22', this)"
                            class="text-xs text-indigo-400 hover:text-indigo-300 mt-0.5">Copy</button>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="flex items-center gap-3 p-3 bg-[#0f1117] rounded-lg border border-indigo-500/30">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div class="flex gap-3 mt-1">
                        <a href="<?php echo html_escape($acc['url']); ?>" target="_blank" rel="noopener noreferrer"
                           class="text-xs text-indigo-400 hover:text-indigo-300 font-medium flex items-center gap-1">
                            <i data-lucide="external-link" class="w-3 h-3"></i> Open Dashboard
                        </a>
                        <button type="button" onclick="copyText('<?php echo html_escape($acc['url']); ?>', this)"
                                class="text-xs text-slate-500 hover:text-slate-300">Copy URL</button>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
            ...
            <div class="flex items-center gap-3 p-3 bg-[#0f1117] rounded-lg border border-[#2a2d3e]">
                ...
                <div class="min-w-0">
                    ...
                    <a href="http://<?php echo html_escape($resolvedIp); ?>/" target="_blank" rel="noopener noreferrer"
                       class="text-xs text-green-400 hover:text-green-300 flex items-center gap-1 mt-1">
                        <i data-lucide="external-link" class="w-3 h-3"></i> Open
                    </a>
                    <p class="text-xs text-slate-600 mt-0.5">Only works if a web server is running on this VPS</p>
                </div>
            </div>
            <?php endif; ?>
        ...
        <div class="mt-4 p-3 bg-[#0f1117] rounded-lg border border-yellow-500/20 flex flex-wrap items-center gap-4">
            <div>
                <p class="text-xs text-slate-500 mb-0.5">Login Username</p>
                <p class="text-sm text-slate-200 font-mono select-all">root</p>
            </div>
            <div>
                <p class="text-xs text-slate-500 mb-0.5">Root Password</p>
                <p class="text-sm text-slate-200 font-mono select-all" id="rootPassDisplay">••••••••</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-[#13151f] border border-[#2a2d3e] rounded-xl p-5 lg:col-span-2 space-y-0">
            ...
            <?php
                ...
                if ($resolvedAppName) {
                    $rows[] = array('Installed App', html_escape($resolvedAppName), 'app');
                }
                ...
                if ($resolvedIp) {
                    $rows[] = array('IPv4',          '<span class="font-mono select-all">'.html_escape($resolvedIp).'</span>', 'raw');
                } else {
                    $rows[] = array('IPv4',          '<span class="text-slate-500">Pending...</span>', 'raw');
                }
                if (!empty($provider_ipv6)) {
                ...
                if ($provider_uptime) {
                    $rows[] = array('Uptime',        html_escape($provider_uptime));
                }
            ...
            <div class="mt-6 border-t border-[#1e2130] pt-6">
                ...
                <form method="post" action="<?php echo site_url('client/services/'.(int) $service['id'].'/reinstall'); ?>" class="space-y-3" id="reinstallForm">
                    <div>
                        ...
                        <select name="os" id="osSelect" class="w-full bg-[#0f1117] border border-[#2a2d3e] rounded-lg px-3 py-2 text-slate-200 text-sm">
                            ...
                            <?php if (!empty($available_os) && is_array($available_os)): ?>
                                <?php foreach ($available_os as $os): ?>
                                    <?php
                                        $osId  = (int) (isset($os['id']) ? $os['id'] : 0);
                                        $osLbl = '';
                                        foreach (array('label','name','title','version_name') as $ok) {
                                            if (!empty($os[$ok])) { $osLbl = $os[$ok]; break; }
                                        }
                                        if (!$osLbl) $osLbl = 'OS #'.$osId;
                                    ?>
                        ...
                        <?php if (!empty($os_dropdown_note)): ?>
                            <p class="text-xs text-slate-500 mt-1"><?php echo html_escape($os_dropdown_note); ?></p>
                        <?php endif; ?>
                    </div>
                    <div>
                            ...
                            <?php if (!empty($applications) && is_array($applications)): ?>
                                <?php foreach ($applications as $app): ?>
                                    <?php
                                        ...
                                        $appLbl = '';
                                        foreach (array('label','name','title') as $ak) {
                                            if (!empty($app[$ak])) { $appLbl = $app[$ak]; break; }
                                        }
                                        if (!$appLbl) $appLbl = 'App #'.$appId;
                                    ?>
                    ...
                    <div>
                        ...
                        <input name="password" type="text" minlength="8" maxlength="64" autocomplete="new-password"
                               placeholder="Leave blank to keep current"
                               class="w-full bg-[#0f1117] border border-[#2a2d3e] rounded-lg px-3 py-2 text-slate-200 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs text-slate-400 mb-1">Application Data <span class="text-slate-600">(JSON, optional)</span></label>
                        <textarea name="application_data" id="applicationData" rows="3"
                                  class="w-full bg-[#0f1117] border border-[#2a2d3e] rounded-lg px-3 py-2 text-slate-200 text-sm font-mono"
                                  placeholder='{"domain":"example.com","password":"StrongPass123"}'></textarea>
                    </div>
                    <button type="submit"
                            class="px-4 py-2 <?php echo $providerBusy ? 'bg-[#373746] text-slate-500 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-500 text-white'; ?> text-sm rounded-lg transition-colors"
                            <?php echo $providerBusy ? 'disabled' : ''; ?>>
                        Reinstall / Install App
                    </button>
                </form>
            </div>
            ...
            <div class="mt-6 border-t border-[#1e2130] pt-6">
                ...
                <form method="post" action="<?php echo site_url('client/services/'.(int) $service['id'].'/change_password'); ?>" class="flex gap-2 items-end">
                    <div class="flex-1">
                        <label class="block text-xs text-slate-400 mb-1">New Password</label>
                        <input name="password" type="text" minlength="8" maxlength="64" required autocomplete="new-password"
                               placeholder="Min 8 characters"
                               class="w-full bg-[#0f1117] border border-[#2a2d3e] rounded-lg px-3 py-2 text-slate-200 text-sm">
                    </div>
                    <button type="submit"
                            class="px-4 py-2 <?php echo $providerBusy ? 'bg-[#373746] text-slate-500 cursor-not-allowed' : 'bg-teal-700 hover:bg-teal-600 text-white'; ?> text-sm rounded-lg transition-colors shrink-0"
                            <?php echo $providerBusy ? 'disabled' : ''; ?>>
                        Change Password
                    </button>
                </form>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-[#13151f] border border-[#2a2d3e] rounded-xl p-5">
                <h3 class="text-lg font-semibold text-white mb-4">
                    Resources
                    <span class="ml-1 text-xs font-normal <?php echo $resLive ? 'text-green-400' : 'text-slate-500'; ?>">
                        (<?php echo $resLive ? 'Real-time' : 'Local'; ?>)
                    </span>
                </h3>
                <div class="space-y-3">
                    <div class="flex items-center gap-3 p-3 bg-[#0f1117] rounded-lg">
                        <i data-lucide="zap" class="w-4 h-4 text-yellow-400 shrink-0"></i>
                        <div><p class="text-xs text-slate-500">CPU</p><p class="text-lg font-semibold text-white"><?php echo $vcpu; ?> vCPU</p></div>
                    </div>
                    <div class="flex items-center gap-3 p-3 bg-[#0f1117] rounded-lg">
                        <i data-lucide="memory-stick" class="w-4 h-4 text-blue-400 shrink-0"></i>
                        <div><p class="text-xs text-slate-500">RAM</p><p class="text-lg font-semibold text-white"><?php echo $memory >= 1024 ? round($memory/1024, 1).' GB' : $memory.' MB'; ?></p></div>
                    </div>
                    <div class="flex items-center gap-3 p-3 bg-[#0f1117] rounded-lg">
                        <i data-lucide="hard-drive" class="w-4 h-4 text-green-400 shrink-0"></i>
                        <div><p class="text-xs text-slate-500">Disk</p><p class="text-lg font-semibold text-white"><?php echo $disk; ?> GB</p></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-[#13151f] border border-[#2a2d3e] rounded-xl p-5">
                ...
                <?php
                    $pct = $provider_bandwidth_limit > 0 ? min(100, round(($provider_bandwidth_used / $provider_bandwidth_limit) * 100)) : 0;
                    $bwColor = $pct >= 90 ? 'bg-red-500' : ($pct >= 70 ? 'bg-yellow-500' : 'bg-indigo-500');
                ?>
                <div class="mb-2 flex justify-between text-xs text-slate-400">
                    <span>Used: <?php echo number_format($provider_bandwidth_used, 1); ?> GB</span>
                    <span>Limit: <?php echo number_format($provider_bandwidth_limit, 1); ?> GB</span>
                </div>
                <div class="w-full bg-[#0f1117] rounded-full h-2">
                    <div class="<?php echo $bwColor; ?> h-2 rounded-full" style="width:<?php echo $pct; ?>%"></div>
                </div>
            ...
            <?php if (!empty($applications) && is_array($applications)): ?>
            <div class="bg-[#13151f] border border-[#2a2d3e] rounded-xl p-5">
                <h3 class="text-base font-semibold text-white mb-3">Available Applications</h3>
                <div id="applicationList" class="max-h-48 overflow-y-auto space-y-1">
                    <?php foreach ($applications as $app): ?>
                        <?php
                            $appId  = (int) (isset($app['id']) ? $app['id'] : 0);
                            $appLbl = '';
                        ...
                        <button type="button"
                                ...
                                data-app-name="<?php echo html_escape($appLbl); ?>"
                                class="app-option w-full text-left text-xs px-3 py-2 rounded-lg bg-transparent hover:bg-[#1c2030] text-slate-300 flex items-center gap-2 transition-colors">
                            <i data-lucide="package" class="w-3 h-3 text-indigo-400 shrink-0"></i>
                            <?php echo html_escape($appLbl); ?>
                        </button>
                    <?php endforeach; ?>
                </div>
                <p class="text-xs text-slate-600 mt-2">Click an app to select it in the install form.</p>
            </div>
            <?php endif; ?>
        </div>
    </div>
...
<script>
(function () {
    // Copy-to-clipboard helper used by access cards
    ...
    var appList   = document.getElementById('applicationList');

    function defaultDataForApp(name) {
        var n = (name || '').toLowerCase();
        if (n.indexOf('directadmin') !== -1) return '{"domain":"example.com","email":"admin@example.com","password":"StrongPass123"}';
        if (n.indexOf('cpanel') !== -1 || n.indexOf('whm') !== -1) return '{"domain":"example.com","password":"StrongPass123"}';
        if (n.indexOf('plesk') !== -1) return '{"domain":"example.com","password":"StrongPass123"}';
        if (n.indexOf('wordpress') !== -1) return '{"domain":"example.com","password":"StrongPass123","email":"admin@example.com"}';
        if (n.indexOf('nextcloud') !== -1) return '{"domain":"cloud.example.com","password":"StrongPass123"}';
        if (n.indexOf('openvpn') !== -1) return '{"domain":"vpn.example.com","password":"StrongPass123"}';
        return '';
    }

    if (appList && appSelect) {
        appList.addEventListener('click', function (e) {
            var btn = e.target.closest('.app-option');
            if (!btn) { return; }
            var id   = btn.getAttribute('data-app-id')   || '';
            var name = btn.getAttribute('data-app-name') || '';
            if (osSelect) { osSelect.value = ''; }
            appSelect.value = id;
            if (appData && !appData.value.trim()) { appData.value = defaultDataForApp(name); }
            // Scroll reinstall form into view
            var form = document.getElementById('reinstallForm');
    ...
    if (appSelect) {
        appSelect.addEventListener('change', function () {
            var selected = appSelect.options[appSelect.selectedIndex];
            if (appSelect.value && osSelect) { osSelect.value = ''; }
            if (appData && !appData.value.trim()) { appData.value = defaultDataForApp(selected ? selected.text : ''); }
        });
    }
    ...
    // Auto-refresh when server is in a transitional state.
